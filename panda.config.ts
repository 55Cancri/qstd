...
export default defineConfig({
  ...
  // Include source files for static extraction
  // NOTE: The commented line below would enable the css() function to work INSIDE the qstd package itself.
  // This is useful if you need to use Panda CSS inline styles within the library code.
    ...
    debug: {
      ...
      transform(value) {
        ...
        if (typeof value === "string") {
          ...
          const isFunc =
            /^(rgb|rgba|hsl|hsla|lab|lch|oklab|oklch|color)\(/i.test(
              borderColor
            );
          ...
          const normalizedBorderColor = shouldWrap
    ...
    cols: {
      transform(value) {
        ...
        if (alignmentValues.includes(templateWords[0])) {
          ...
          gridTemplate = templateWords.join(" ");
        }
