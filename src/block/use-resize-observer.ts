...
// Hook: observe a single DOM element's rendered size via ResizeObserver.
// Returns a ref callback to attach to the target element and exposes width/height.
// If `onResize` is provided, it will be called with size updates and internal
// state will NOT be updated (callback mode). Otherwise, the hook updates state.
// Supports RO `box` option and a customizable `round` function to stabilize values.
  ...
  const refCallback = useResolvedElement<T>(
    React.useCallback(
      (element) => {
        ...
        // Lifecycle: (re)create the RO if `box` or `round` changed; otherwise reuse.
        if (
          ...
          resizeObserverRef.current = {
            ...
            instance: new ResizeObserver((entries) => {
              ...
              // Skip updates if nothing changed; call user callback if provided,
              // otherwise update internal state (state mode).
              if (
        ...
        // Begin observing the resolved element with the requested `box` option.
        resizeObserverRef.current.instance.observe(element, { box: opts.box });

        return () => {
          if (resizeObserverRef.current) {
            // Stop observing this element; RO instance may be reused for next element.
...
// Helper: resolves the element to observe from three inputs — the callback ref
// (returned by this hook), an external RefObject, or a direct element — and
// manages subscribe/cleanup when either the element or the subscriber changes.
function useResolvedElement<T extends Element>(
  subscriber: (element: T) => SubscriberResponse,
  refOrElement?: T | React.RefObject<T> | null
): React.RefCallback<T> {
  ...
  const evaluateSubscription = React.useCallback(() => {
    ...
    if (
      ...
      // No changes; keep current subscription.
      return;
    }
